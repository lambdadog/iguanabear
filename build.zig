...
pub fn libiguanabear(
    b: *Builder,
    target: CrossTarget,
    mode: Mode,
    comptime prefix_path: []const u8,
) *LibExeObjStep {
    // It's not exactly a "main" file, but shim.zig is our only zig
    // file in the build...
    const lib = b.addStaticLibrary("iguanabear", "libc/shim.zig");
    lib.setBuildMode(mode);
    lib.setTarget(target);

    const bearssl_path = prefix_path ++ "BearSSL/";

    // headerspub:
    //   inc/bearssl.h inc/bearssl_aead.h inc/bearssl_block.h
    //   inc/bearssl_ec.h inc/bearssl_hash.h inc/bearssl_hmac.h
    //   inc/bearssl_kdf.h inc/bearssl_pem.h inc/bearssl_prf.h
    //   inc/bearssl_rand.h inc/bearssl_rsa.h inc/bearssl_ssl.h
    //   inc/bearssl_x509.h
    lib.addIncludeDir(bearssl_path ++ "inc/");

    // headerspriv:
    //   src/config.h src/inner.h
    lib.addIncludeDir(bearssl_path ++ "src/");

    lib.addSystemIncludeDir(prefix_path ++ "libc/include/");
    ...
    inline for (csources) |csrc| {
        lib.addCSourceFile(bearssl_path ++ csrc, &[_][]const u8{
            "-W",
            "-Wall",
            "-Os",
            "-fPIC",
            // Easier to seed from zig wrapper than it is to correctly
            // implement the increased subset of libc required for
            // urandom/win32 rand...
            "-DBR_RDRAND=0",
            "-DBR_USE_URANDOM=0",
...
pub fn build(b: *std.build.Builder) void {
    ...
    const iguanabear = libiguanabear(
        b,
        target,
        mode,
        "",
